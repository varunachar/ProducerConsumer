...
public class Queue
{
	...
	private BlockingQueue<Runnable> blockingQueue = new ArrayBlockingQueue<Runnable>(
			CAPACITY);;
			private ExecutorService service = new ThreadPoolExecutor(CORE_POOL_SIZE,
					MAX_POOL_SIZE, KEEP_ALIVE_TIME, TIME_UNIT, this.blockingQueue);
			/**
			 ...
			 */
			public Queue(final RejectionStrategy rejectionStrategy,
			...
			public void shutdown()
			{
				...
				{
					log.error("Could not await termination of queue because "
							+ e.getMessage(), e);
				}
			}
